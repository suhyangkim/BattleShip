public class Grid{
   ...
   public void addShip(Ship s){
      ...
      //if horizontal, set ship across the row
      if(direction == 0){
         for(int r = row; r < (row+length); r++){
            Location curr = grid[r][col];
            curr.setShip(true);
         }
      } else{
         //if vertical, set ship down the col
         for(int c = col; c < (col+length); c++){
            Location curr = grid[row][c];
            curr.setShip(true);
         }
      }
   }
   ...
   public void printStatus(){
      ...
      for(int row = 0; row < 10; row++){
         //print alphabet
         Character current = (char)(((int)'A' ) + row);
         System.out.print(current);
         //print status for each location in the current row
         for(int col = 0; col < 10; col++){
            Location curr = grid[row][col];
            if(curr.checkHit()){
               System.out.print(" X");
   ...
   public void printShips(){
      ...
      for(int row = 0; row < 10; row++){
         ...
         Character current = (char)(((int)'A' ) + row);
         System.out.print(current);
         //print whether there is a ship for each location in the current row
         for(int col = 0; col < 10; col++){
            Location curr = grid[row][col];
            if(curr.hasShip()){
               System.out.print(" X");
